// assignments/04-mongo-db/02-documents-and-indexes/01-db-setup/01_collections_and_indexes.js
// Purpose: Create collections with JSON Schema validators and build indexes (idempotent).
// How to run:
...
// Helper: create or update validator and ensure strict validation.
  ...
  $jsonSchema: {
    ...
    properties: {
      ...
      email: {
        ...
        maxLength: 254,
        pattern: '^[^\s@]+@[^\s@]+\.[^\s@]+$',
        description: 'unique user email (basic regex)',
      },
      password_hash: { bsonType: 'string', minLength: 6, maxLength: 255 },
...
const stepSchema = {
  $jsonSchema: {
    ...
    properties: {
      ...
      name: { bsonType: 'string', maxLength: 50 },
      url: { bsonType: ['string', 'null'], maxLength: 512, pattern: '^https?://' },
...
print('✔ Validators ensured and indexes created.');
